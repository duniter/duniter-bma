...
function WOTBinding (server) {
  ...
  const Identity = server.lib.Identity;
  ...
  this.requirementsOfPending = (req) => co(function *() {
    const minsig = ParametersService.getMinSig(req)
    const identities = yield server.dal.idtyDAL.query('SELECT i.*, count(c.sig) as nbSig FROM idty i, cert c WHERE c.target = i.hash group by i.hash having nbSig >= ?', minsig)
    const all = yield BlockchainService.requirementsOfIdentities(identities);
    if (!all || !all.length) {
      throw constants.ERRORS.NO_IDTY_MATCHING_PUB_OR_UID;
    }
