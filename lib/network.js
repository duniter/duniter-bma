...
module.exports = {
  ...
  createServersAndListen: (name, server, interfaces, httpLogs, logger, staticPath, routingCallback, listenWebSocket, enableFileUpload) => co(function *() {
...
function getBestLocalIPv6() {
  ...
  for (let netInterface of osInterfaces) {
    ...
    const filtered2 = _.filter(filtered, (address) => !address.address.match(/^fe80/) && !address.address.match(/^::1/));
    if (filtered2[0]) {
